...
export function modelSwitchCacheWarning(locale: UiLocale): string {
  return getTuiCopy(locale).modelSearch.cacheWarning;
}
...
export class ModelSearchOverlay implements Component {
  ...
  render(width: number): string[] {
    const safeWidth = Math.max(1, width);
    const copy = getTuiCopy(this.input.locale).modelSearch;
...
export function onboardingProviderPickerItems(
  ...
  const configured = getTuiCopy(locale).providerPicker.configured;
...
export function thinkingLevelPickerItems(
  ...
  const copy = getTuiCopy(locale).thinking;
  return [
    {
      value: 'default',
      label: copy.default,
...
export class OnboardingWizard implements Component {
  ...
  private handleModelsInput(data: string): void {
    ...
    if (matchesKey(data, Key.enter) || matchesKey(data, Key.return)) {
      ...
      if (this.selectedIds.size === 0) {
        this.status = {
          kind: 'error',
          text: getTuiCopy(this.input.locale).wizard.selectAtLeastOneModel,
        };
        return;
      }
  ...
  private renderSearch(width: number): string[] {
    const copy = getTuiCopy(this.input.locale).wizard;
    this.searchEditor.focused = true;
  ...
  private renderSuccess(width: number): string[] {
    ...
    const label = this.picked?.label ?? '';
    return [
      padLine(`Set Up Provider ${ansi.dim(`· ${copy.doneStep}`)} ${ansi.accent(label)}`, width),
